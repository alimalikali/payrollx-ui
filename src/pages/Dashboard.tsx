...
import { useAIDashboardStats, useAttendance, useDailyStats, usePayrollForecast } from "@/hooks";

const mapHeatmapStatus = (status: string): "present" | "absent" | "late" | "leave" | "weekend" | "holiday" => {
    ...
    <AppShell>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Live overview connected to PayrollX APIs.</p>
        </div>

        {(attendanceQuery.isLoading || dailyStatsQuery.isLoading || aiDashboardQuery.isLoading) && (
          <p className="text-sm text-muted-foreground">Loading dashboard data...</p>
        )}

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard title="Total Employees" value={totalEmployees} icon={Users} />
          <KPICard title="Payroll Cost" value={payrollCost.toLocaleString()} prefix="PKR" icon={DollarSign} />
          <KPICard title="Active Alerts" value={activeAlerts} icon={Bell} trend={"Live"} />
          <KPICard title="Attendance Rate" value={`${attendanceRate}%`} icon={BarChart2} trend={"Today"} />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <ChartCard title="Payroll Forecast" subtitle="Projected gross and net payroll" className="lg:col-span-3">
            <div className="h-[300px] lg:h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis dataKey="month" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} />
                  <YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} tickFormatter={(value) => `${value}M`} />
                  <Tooltip
                    contentStyle={{
                  ...
                  />
                  <Line type="monotone" dataKey="projected" stroke="hsl(var(--chart-1))" strokeWidth={2} name="Gross" />
                  <Line type="monotone" dataKey="net" stroke="hsl(var(--chart-2))" strokeWidth={2} strokeDasharray="5 5" name="Net" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </ChartCard>

          <ChartCard title="Attendance Heatmap" subtitle="Latest attendance entries" className="lg:col-span-2">
